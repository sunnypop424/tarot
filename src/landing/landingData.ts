...
export const STEPS: Step[] = [
  ...
  },
  {
    ...
    name: '현장 운영',
    tag: '스태프 기기는 전날 준비',
    desc: '준비된 페이지를 현장에서 바로 사용하시면 됩니다. 스태프가 쓰는 화면은 전날 미리 열어 "홈 화면에 추가"로 앱처럼 띄우고 로그인까지 해 두시면 당일이 수월합니다. 운영 중 예기치 못한 문제가 발생하면 빠르게 해결해 드립니다.',
  },
]
...
export const FAQS = [
  ...
  },
  {
     ...
     */
    a: '카페를 방문하신 팬분들은 설치가 전혀 필요 없습니다. QR 코드를 스캔하면 기본 브라우저에서 바로 열립니다. 다만 스태프만 사용하는 화면(럭키드로우 뽑기, 포토카드 증정·판매, 스탬프 수령 확인 등)은 행사 전날 미리 열어 브라우저의 "홈 화면에 추가"로 앱처럼 띄워 두시길 권해 드립니다. 주소창 없이 바로 열려 현장에서 주소를 찾을 일이 없고, 로그인도 미리 해 둘 수 있습니다.',
  },
  {
