...
/**
 * 스태프 기기를 쓰는 서비스에 공통으로 붙는 안내.
 *
 * 손님 화면은 QR 로 열면 그만이라 설치가 없다는 게 요점인데, **스태프 화면은 반대다** —
 * 현장에서 주소를 치거나 링크를 찾다 사고가 난다. 전날 홈 화면에 붙이고 로그인까지 해 두는
 * 게 실제 운영에서 제일 크게 갈린다. 럭키드로우 안내(`owner/guide.ts`)엔 이미 있던 문장을
 * 문의 단계로 끌어올린 것이다.
 */
const STAFF_DEVICE_NOTE =
  '※ 스태프 기기는 행사 전날 미리 열어 브라우저의 "홈 화면에 추가"로 앱처럼 띄우고, 로그인까지 해 두시길 권해 드립니다.'
...
export const SERVICE_FORM: Record<ServiceId, { name: string; lines: string[] }> = {
  ...
  photozone: {
    ...
    lines: [
      ...
      '기본 카메라 (전면 / 후면):',
      '결과물에 넣을 워터마크 문구:',
      '※ 손님 사진은 폰 안에서 합성되고 서버에 올라가지 않습니다.',
    ],
  },
  cheer: {
    name: '영상회 응원',
...
export const CUSTOM_FORM = {
  ...
  lines: [
    ...
    '조작하는 사람 (손님 폰 / 스태프 기기 / 스크린 — 여럿이면 다 적어 주세요):',
