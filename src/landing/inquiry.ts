/**
 * 문의 양식 — **방문자가 오픈채팅에 붙여넣을 글**을 여기서 만든다.
 *
 * 문의는 오픈채팅으로만 받는다. 폼을 서버로 보내지 않는 게 요점이라 이 파일은 **문자열만**
 * 만들고, 보내는 일은 방문자의 카톡이 한다 (우리 쪽에 저장되는 개인정보가 0이 된다).
 *
 * 서비스별 칸은 **실제로 우리가 받아야 하는 것만** 적는다. 관리자 페이지에서 방문자가 직접
 ...
 *
 * 방문자 화면은 QR 로 열면 그만이라 설치가 없다는 게 요점인데, **스태프 화면은 반대다** —
