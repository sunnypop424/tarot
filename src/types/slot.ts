...
export interface Slot {
  ...
  name: string
  /**
   * 묶음 이름 — **최고관리자의 정리 도구다** (`0028_slot_group.sql`).
   *
   * 한 생일카페가 포토카드·스탬프·모의고사를 같이 얹으면 슬롯이 셋인데, 목록에서 남의 행사와
   * 뒤섞였다. 같은 이름을 적으면 목록에서 한 덩어리로 묶인다.
   *
   * **권한이 아니다.** 접근은 지금처럼 slug 스코프고, 같은 묶음이라고 주최자가 옆 슬롯을
   * 볼 수 있게 되지 않는다.
   */
  group?: string
  /**
  ...
  demo?: boolean
  /**
   * 방문자에게 보여줄 **언어들** — `'en'` · `'zh'` · `'ja'` (`src/i18n/index.tsx` 의 `LANGS`).
   *
   * **한국어는 안 넣는다.** 늘 있는 기본값이라 목록에 넣으면 "빼면 한국어가 사라지나" 가 된다.
   * 화면이 앞에 붙여서 그린다.
   *
   * 비어 있으면(기본) **언어 단추가 아예 안 뜬다.** 해외 팬이 안 오는 행사에 단추가 떠 있으면
   * 눌러볼 거리일 뿐이고, 눌러도 **주최자가 쓴 내용(질문·상품명·문항·쪽지)은 한국어 그대로**라
   * 반쪽만 번역된 화면을 보게 된다. 그 한계를 아는 주최자만 켜는 게 맞다.
   *
   * **관리 화면 토글은 이 값과 무관하게 늘 넷 다 있다** — 그건 "방문자에게 무엇을 보여줄까" 가
   * 아니라 "이 화면을 쓰는 사람이 한국어를 읽나" 라 다른 질문이다.
   */
  langs?: string[]
  /**
   * **처음 열었을 때 뜨는 언어** — 없으면 한국어.
   *
   * 일본 팬 위주 행사라면 일본어로 열리는 게 맞다. 매번 지구본을 눌러 바꾸게 하면
   * 그 한 번을 안 누르는 사람이 대부분이고, 그 사람들에게 이 행사는 한국어 행사다.
   *
   * **방문자가 한 번이라도 직접 고르면 그쪽이 이긴다** — 이 값은 "처음" 에만 쓴다
   * (`LangProvider.applyDefault`). 안 그러면 한국어로 바꿔 놓고 새로고침할 때마다
   * 일본어로 돌아간다.
   *
   * `langs` 에 없는 값이면 무시한다 — 고를 수 없는 언어로 열어 두면 되돌릴 길이 없다.
   */
  defaultLang?: string
  /**
