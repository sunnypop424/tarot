...
export function Stats() {
  ...
  const head = (
    <header className="ad-head">
      <div className="ad-head__row">
        <h1 className="ad-head__title">{t('통계')}</h1>
      ...
      <>
        ...
        <div className="ad-card">
          <div className="ad-empty">
            <div className="ad-empty__title">{t('지금 빌드에서는 통계를 쓸 수 없어요')}</div>
          </div>
        </div>
      </>
    )
  ...
  async function rescore() {
    const ok = await confirmAction({
      title: t('다시 채점할까요?'),
      desc: t('이미 응시한 분들의 점수도 함께 고쳐져요. 인정할 답을 더한 뒤에 눌러 주세요.'),
      okLabel: t('다시 채점'),
    })
    if (!ok) return
    setBusy(true)
    try {
      ...
      await load()
      toast(n > 0 ? `${n}명의 점수를 다시 매겼어요` : t('바뀐 점수가 없어요'))
    } catch (e) {
      toast(e instanceof Error ? e.message : t('다시 채점하지 못했어요'))
    } finally {
      setBusy(false)
    }
  }
      ...
      <div className="ad-stack">
        <div className="ad-stats">
          <div className="ad-stat">
            <div className="ad-stat__label">{t('응시')}</div>
          ...
          <div className="ad-stat">
            <div className="ad-stat__label">{t('평균')}</div>
            <div className="ad-stat__row">
          ...
          <div className="ad-stat">
            <div className="ad-stat__label">{t('가장 어려운 문항')}</div>
            <div style={{ fontSize: 15, fontWeight: 700, marginTop: 10, lineHeight: 1.5 }}>
              {hardest ? hardest.body : t('아직 없어요')}
            </div>
          </div>
        </div>
        ...
        <div className="ad-card">
          <div className="ad-card__head" style={{ marginBottom: 16 }}>
            <span className="ad-card__title">{t('문항별 정답률')}</span>
            <button
              type="button"
