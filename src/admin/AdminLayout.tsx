  ...
  return [
    {
      ...
      items: [
        { to: '', label: '대시보드' },
        /**
         * QR — **인쇄물 준비가 주최자의 첫 번째 일이다.** 다만 럭키드로우는 빼야 한다.
         *
         * QR 은 "방문자가 찍고 들어오는 주소" 를 인쇄물로 만드는 도구인데, 럭키드로우의
         * 그 주소는 **부스 태블릿에서 스태프가 여는 화면**이다 (`LuckydrawApp` 머리말).
         * 붙여 두면 방문자가 찍고 들어와 **아무것도 못 하는 화면**을 본다 — 로그인이
         * 없으면 뽑기 버튼이 안 눌린다. 게다가 재고·경품 목록이 그대로 보인다.
         */
        ...(service === 'luckydraw' ? [] : [{ to: 'qr', label: 'QR 만들기' }]),
      ],
    },
