...
const ADMIN_ROUTES: Record<ServiceId, ReactNode> = {
   ...
   * 부스 기기에 관리 사이드바가 같이 뜨면 방문자가 누르고, 방문자가 같이 보는 화면이라
    ...
    <Routes>
      ...
      <Route element={<RequireAuth />}>
        <Route element={<AdminLayout />}>
          ...
          {/*
            * QR 은 주소가 있는 슬롯이면 다 필요하다 — **럭키드로우만 뺀다.**
            *
            * 그 슬롯의 주소는 방문자가 찍고 들어오는 곳이 아니라 **부스 태블릿에서 스태프가
            * 여는 화면**이라, 인쇄해 붙이면 방문자가 아무것도 못 하는 화면(로그인 없이는
            * 뽑기 버튼이 안 눌린다)에 들어와 재고와 경품 목록만 보게 된다.
            *
            * **메뉴만 숨기면 주소를 직접 친 사람에게는 그대로 열린다** — 계정 화면들이
            * 바로 아래에서 같은 이유로 라우트째 빠지는 것과 같다. 여기서 빼면 아래
            * `path="*"` 가 받아 **대시보드로 보낸다** (옛 주소를 북마크했어도 빈 화면이 아니다).
            */}
          {service !== 'luckydraw' && <Route path="qr" element={<Qr />} />}
          {/*
            계정 두 화면은 **체험 슬롯에서 빠진다** — 메뉴만 숨기면 주소를 직접 치면 열린다.
            '스태프 계정' 은 진짜 Supabase 계정을 만들고(매시간 되돌리기가 못 지운다),
            '내 계정' 은 바꿀 비밀번호가 있는 세션이 전제인데 체험엔 세션이 없다.
          */}
          {!slot.demo && <Route path="staff-accounts" element={<Staff />} />}
          {ADMIN_ROUTES[service]}
          {/* local 어댑터엔 바꿀 비밀번호가 없다 (아무 값이나 통과한다) — 메뉴도 화면도 안 만든다 */}
          {hasSupabase && !slot.demo && <Route path="account" element={<Account />} />}
        </Route>
