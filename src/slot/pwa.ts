 ...
 *
 * 방문자는 QR 로 들어와 한 번 쓰고 말지만, **스태프는 기기를 들고 하루를 난다** — 그래서
 ...
 * "홈 화면 추가" 만 목적이다.
 *
 * **지금 보고 있는 영역으로 열린다.** 예전엔 어디서 추가하든 `start_url` 이 방문자 화면이라,
 * 스태프가 뽑기 화면을 붙여 놓고 눌러도 방문자 화면이 떴다 — 붙여 두는 이유가 사라지는 버그였다.
 * `scope` 는 셋 다 `/{slug}` 로 둔다(관리 화면이 방문자 화면으로 넘어가도 앱 안에 머문다).
 *
