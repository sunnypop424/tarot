...
// wash/primary 만 다르게 주는 슬롯 (나머지는 데모 기본)
const slot = (over) => ([{
  slug: 'demo', name: '데모 생일카페',
  theme: {
    colors: {
...
const run = async (label, over, shot) => {
  ...
  await page.evaluate((s) => localStorage.setItem('tarot-pocket:slots-draft', JSON.stringify(s)), slot(over))
